...
@Controller
	...
	패키지를 스캔할 때 해당 어노테이션이 있는 클래스를 찾아서 요청을 전달
	
@RequestMapping
	: 요청명을 매핑한다. 요청명은 서블릿과 마찬가지고 컨텍스트루트를 제외한
	나머지 경로명으로 이루어진다. 요청명의 매핑정보로 메서드를 호출하므로 
	메서드명은 큰 의미가 없다. 단지 개발자가 구분하기 좋은 이름으로 설정한다.
 */
...
public class FormController {
	...
	/*
	폼값받기1] 파라미터로 전달된 값을 HttpServletRequest 내장객체로 받는다.
	...
	파라미터를 마치 경로명처럼 사용하게 되고, 이때 주의해야할 점은
	웹브라우저는 요청명을 경로로 인식하므로 리소스를 사용할 때 경로를
	주의해야한다. 또한 파라미터의 수가 다르면 404에러가 발생한다.
	 */
