...
public class RequestMappingController {
	...
	public String getSearch(HttpServletRequest req, Model model, HttpSession session) {
		
		session.setAttribute("userId", "내아디!");//걍 설명용으로 넣은 세션씨
		/*
		요청 처리를 위한 메서드를 정의할 때 해당 메서드에서 사용하고자 하는
		내장객체가 있다면 매개변수형태로 선언하면 즉시 사용할 수 있다.
		즉, 컨트롤러에 정의되는 메서드는 매개변수의 개수가 큰의미를 가지지않음.
		 */
	...
	public ModelAndView postLogin(
		...
		*/
			...
			/*
			ModelAndView객체를 반환하여 뷰를 호출한다. 따라서 
			메서드의 반환 타입도 동일객체로 지정해야한다.
			 */
			return mv;
	}
	...
	/*
	@ModelAttribute어노테이션
		:뷰로 전달되는 커맨드 객체의 이름을 임의로 변경하고 싶을 때 사용
